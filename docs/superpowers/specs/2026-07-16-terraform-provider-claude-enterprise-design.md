...
- `GET /v1/organizations/spend_limits/effective` — 멤버별 실효 한도, `source`(상속원),
  `period_to_date_spend`, `actor.user_id`, `actor.email_address`. opaque cursor
  페이지네이션(`next_page` → `page`), 시퀀스 중간에 쿼리 파라미터 변경 시 400.
...
- 금액은 조직 청구통화 최소 단위(cents)의 문자열. `"50000"` = 500.00 USD. `null` =
  무제한, `"0"` = 플랜 포함분만.
- `period`는 현재 `monthly`만 지원하나 open set으로 취급.
...
| `id` | string | Computed | `spend_limit_id` |
| `currency` | string | Computed | 조직 청구통화 |
| `source` | string | Computed | 정보용 |
| `period_to_date_spend` | string | Computed | 정보용. drift 판정에 사용하지 않음 |

- `user_id` XOR `user_email`: 스키마 validator로 강제(정확히 하나 필수).
- `user_id`, `user_email`, `period` 변경은 RequiresReplace. `amount` 변경은 in-place
  update(upsert).
...
- **Import**: `spend_limit_id` 직접 지정 또는 `user_id`(`user_` 접두사로 판별 →
  `/effective`에서 해당 행 검색) 지원.

### email → user_id 해소 (`resolver.go`)

- provider 인스턴스 레벨 캐시: 최초 해소 요청 시 `/effective` 전체를 1회 순회해
  email → user_id 맵을 구축하고 apply 세션 동안 재사용(sync.Once/singleflight).
  리소스가 100개여도 목록 순회는 1번 → 레이트리밋 보호.
- 해소 실패(email이 조직에 없음)는 명확한 에러로 apply 중단.
- email 비교는 대소문자 무시(normalize to lower).

## 7. 데이터소스 `claude-enterprise_members`

- `/effective`를 전체 페이지네이션 순회해 조직 멤버 목록 노출.
- 출력: `by_email`, `by_user_id` — 각각 map of object
  `{ user_id, email, effective_amount, source, period_to_date_spend }`.
...
- `period_to_date_spend`는 일시적으로 `"0"`이 될 수 있으므로 정보용 computed로만 두고
  plan diff에 영향을 주지 않게 처리(UseStateForUnknown 미적용, 항상 API 값 반영하되
  정보성 속성으로 문서화).
...
- `ci.yml` (PR/push): `go build`, `golangci-lint`, `go test ./...`(acceptance 포함,
  mock 기반), `terraform fmt -check`(examples), `tfplugindocs` 생성물 drift 체크.
